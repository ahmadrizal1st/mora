# 🚀 Visatamora — Deployment Guide (Ubuntu 24 LTS)
...
multipass mount /Users/macbook/Documents/visatamora visatamora:/home/ubuntu/visatamora
```
...
CREATE USER visatamora WITH PASSWORD 'ganti_password_kuat_ini';
CREATE DATABASE vistamora OWNER visatamora;
GRANT ALL PRIVILEGES ON DATABASE vistamora TO visatamora;
\q
```
...
sudo mkdir -p /var/www/visatamora
sudo chown $USER:$USER /var/www/visatamora

# Clone repo
cd /var/www/visatamora
git clone https://github.com/your-username/visatamora.git .
```
...
```bash
cd /var/www/visatamora/apps/api
...
```env
APP_NAME="Visatamora API"
APP_ENV=production
APP_KEY=                          # akan di-generate
APP_DEBUG=false
APP_URL=https://api.visatamora.com

FRONTEND_URL=https://visatamora.com
...
sudo chown -R www-data:www-data /var/www/visatamora/apps/api/storage
sudo chown -R www-data:www-data /var/www/visatamora/apps/api/bootstrap/cache
sudo chmod -R 775 /var/www/visatamora/apps/api/storage
sudo chmod -R 775 /var/www/visatamora/apps/api/bootstrap/cache
```
...
```bash
cd /var/www/visatamora/apps/personal
...
VITE_API_URL=https://api.visatamora.com/api
EOF
...
```bash
sudo nano /etc/nginx/sites-available/visatamora-api
```

```nginx
server {
    listen 80;
    server_name api.visatamora.com;
    root /var/www/visatamora/apps/api/public;
    index index.php;
    ...
    access_log /var/log/nginx/visatamora-api-access.log;
    error_log  /var/log/nginx/visatamora-api-error.log;
}
```
...
```bash
sudo nano /etc/nginx/sites-available/visatamora-frontend
```

```nginx
server {
    listen 80;
    server_name visatamora.com www.visatamora.com;
    root /var/www/visatamora/apps/personal/dist;
    index index.html;
    ...
    access_log /var/log/nginx/visatamora-frontend-access.log;
    error_log  /var/log/nginx/visatamora-frontend-error.log;
}
```
...
```bash
sudo nano /etc/supervisor/conf.d/visatamora-queue.conf
```

```ini
[program:visatamora-queue]
process_name=%(program_name)s_%(process_num)02d
command=php /var/www/visatamora/apps/api/artisan queue:work --sleep=3 --tries=3 --max-time=3600
autostart=true
autorestart=true
stopasgroup=true
killasgroup=true
...
stdout_logfile=/var/log/supervisor/visatamora-queue.log
stopwaitsecs=3600
```
...
```bash
sudo nano /etc/nginx/sites-available/visatamora-all
```
...
```bash
sudo ln -s /etc/nginx/sites-available/visatamora-all /etc/nginx/sites-enabled/
...
- [ ] Poppler terinstall
- [ ] PostgreSQL berjalan & database `vistamora` dibuat
- [ ] Nginx berjalan
- [ ] Supervisor berjalan
