  ...
  const cssVariables = useMemo(() => {
    ...
    if (chartData.series) {
      chartData.series.forEach((serie, index) => {
        ...
        vars[`--chart-${resolvedId}-color-${index}`] =
          `color-mix(in srgb, transparent, ${finalColor} ${opacity})`
      })
    }
    ...
    if (chartData.type === 'area' && chartData.series) {
      chartData.series.forEach((serie, index) => {
        ...
        vars[`--chart-${resolvedId}-fill-${index}`] =
          `color-mix(in srgb, transparent, ${finalColor} 16%)`
      })
    }
  ...
  const buildSeries = useCallback(
    (type: string) => {
      ...
      return chartData.series.map((s, index) => ({
        ...
        ...(chartData.types && chartData.types[String(index)]
          ? { type: chartData.types[String(index)] }
          : {}),
      }))
    },
  ...
  const buildLabels = useCallback(
    (type: string) => {
      ...
      if (chartData.datetime) {
        const startDateStr =
          chartData.startDate ?? startDate ?? new Date().toISOString().split('T')[0]
        const start = new Date(startDateStr).getTime()
        const firstSeries = chartData.series[0]
        const daysCount =
          firstSeries?.data?.length ??
          (firstSeries?.candlestickData || firstSeries?.['candlestick-data'])?.length ??
          0
        const labels = []
  ...
  useEffect(() => {
    ...
    const options: ApexOptions = {
      ...
      stroke: {
        show: true,
        width:
          chartData.strokeWidth ??
          chartData['stroke-width'] ??
          (['pie', 'donut'].includes(chartType)
            ? 2
            : ['area', 'line', 'scatter', 'candlestick'].includes(chartType)
              ? chartType === 'candlestick'
                ? 1
                : 2
              : 0),
        colors:
          (chartData.strokeColors ?? chartData['stroke-colors'])
            ? (chartData.strokeColors ?? chartData['stroke-colors'])!.map((c) => resolveCSSColor(c))
            : ['pie', 'donut'].includes(chartType)
              ? [resolveCSSColor('bg-surface')]
              : undefined,
        dashArray: chartData.strokeDash ?? chartData['stroke-dash'] ?? 0,
        lineCap: chartData.lineCap ?? 'round',
        curve: (chartData.strokeCurve ?? chartData['stroke-curve'] ?? 'smooth') as
          | 'smooth'
          | 'straight'
          | 'stepline',
      },
      ...
      ...((chartData.datetime || ['pie', 'donut', 'radialBar'].includes(chartType)) && {
        labels: buildLabels(chartType),
      }),
      tooltip: {
        theme: 'dark',
      ...
      grid: {
        ...
        ...(!chartData.sparkline && {
          padding: {
            top:
              chartType === 'pie' || chartType === 'donut' || chartType === 'radialBar' ? 20 : -20,
            right: 0,
            left: -4,
            bottom: -4,
          },
      ...
      markers: {
        size:
          chartData.showMarkers || chartData['show-markers']
            ? 2
            : chartType === 'scatter'
              ? 4
              : chartData.hidePoints || chartData['hide-points']
                ? 0
                : 0,
      },
    }
    ...
    if (chartData.extend) {
      try {
        const extendOptions =
          typeof chartData.extend === 'string' ? JSON.parse(chartData.extend) : chartData.extend
