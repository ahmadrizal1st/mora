          ...
          {targetItems.map((item, index) => (
            <div key={item.name}>
              ...
              <div className="d-flex gap-1 mb-2">
                <div
                  className={`rounded-pill ${item.colorClass}`}
                  style={{ height: '6px', flex: getProgress(item.current, item.target) }}
                ></div>
                <div
                  className="rounded-pill"
                  style={{
                    height: '6px',
                    flex: 100 - getProgress(item.current, item.target),
