...
// 50/30/20 Rule:
// 50% Kebutuhan  (cash & bank — daily needs)
// 30% Keinginan  (saving  — lifestyle / wants)
// 20% Tabungan   (investment — savings / future)
      ...
      <div className="card-body p-4 pt-0 d-flex flex-column">
        ...
        <div className="mt-2">
          ...
          <div className="d-flex align-items-center gap-2 mb-2">
            <span className="h1 fw-bold mb-0 lh-1" style={{ fontSize: '2rem', letterSpacing: '-0.5px' }}>
          ...
          <div className="d-flex flex-column gap-2">
            {rows.map((row) => (
              <div key={row.label} className="d-flex align-items-center">
                <div className="rounded-1 flex-shrink-0 me-3" style={{ width: '12px', height: '12px', background: row.dotBg }} />
                <div className="flex-grow-1">
                  <div className="d-flex justify-content-between align-items-center mb-0">
                    <div className="fw-bold">{row.label}</div>
                    <div className={`${row.badgeClass} fw-bold rounded-2 px-2 py-0.5`} style={{ background: row.badgeBg, fontSize: '0.75rem' }}>
                      {row.pct.toFixed(1)}%
                    </div>
                  </div>
                  <div className="d-flex align-items-center gap-2">
                    <div className="flex-grow-1 rounded-pill" style={{ height: '4px', background: 'var(--tblr-border-color)' }}>
                      <div
                        className="rounded-pill h-100"
                        style={{ width: `${Math.min((row.pct / row.target) * 100, 100)}%`, background: row.dotBg }}
                      />
                    </div>
                    <div className="text-muted" style={{ fontSize: '0.7rem', whiteSpace: 'nowrap' }}>{row.sub}</div>
                  </div>
                </div>
              </div>
            ))}
