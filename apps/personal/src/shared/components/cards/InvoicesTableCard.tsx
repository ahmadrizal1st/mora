          ...
          <tbody>
            {filtered.slice(0, Number(perPage) || 8).map((inv, i) => (
              <tr key={i} className="text-mobile-xs">
                ...
                <td>
                  <span className="text-secondary">{inv.id?.slice(-3)}</span>
                </td>
          ...
          <div className="row g-2 justify-content-between">
            <div className="col-auto d-flex align-items-center">
              <p className="m-0 text-secondary">
                Showing <strong>1 to {Math.min(filtered.length, Number(perPage) || 8)}</strong> of{' '}
                <strong>{filtered.length} entries</strong>
              </p>
            </div>
