...
export function CommentsCard({
  ...
  comments = [
    ...
    },
    {
      ...
      date: '1 hr ago',
      text: 'Very informative. Looking forward to more content like this.',
      votes: 18,
    },
  ],
  onVoteUp,
      ...
      <div className="card-body">
        {comments.map((c, i) => (
          <div key={i} className={`d-flex${i < comments.length - 1 ? ' mb-5' : ''}`}>
            <div className="me-4">
              <span
                className={`avatar ${i === 0 ? 'bg-green-lt' : ''}`}
            ...
            <div className="flex-fill">
              ...
              <div className="small">
                {c.votes !== undefined && <span className="text-success me-1">+{c.votes}</span>}
                <button
                  className="btn btn-sm btn-ghost-secondary py-0 px-1"
                  onClick={() => onVoteUp?.(i)}
                >
                  ↑
                </button>
                <button
                  className="btn btn-sm btn-ghost-secondary py-0 px-1 me-1"
                  onClick={() => onVoteDown?.(i)}
                >
                  ↓
                </button>
                <span className="me-1">·</span>
                <button
                  className="btn btn-sm btn-link text-secondary py-0 px-0 me-1"
                  onClick={() => onReply?.(i)}
                >
                  Reply
                </button>
