...
export function PlanningSegmentedNav({ activeTab, onTabChange }: PlanningSegmentedNavProps) {
  ...
  return (
    <ul className="nav nav-pills bg-light p-1 rounded-pill border d-inline-flex flex-nowrap shadow-none" style={{ background: '#f8fafc' }}>
      {tabs.map((tab) => (
        <li key={tab.id} className="nav-item">
          <button
            ...
            className={clsx(
              ...
              activeTab === tab.id ? 'active bg-white text-primary shadow-sm fw-bold' : 'text-secondary opacity-75'
            )}
