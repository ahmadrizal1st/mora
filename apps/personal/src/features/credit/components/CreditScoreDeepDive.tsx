...
export function CreditScoreDeepDive() {
  ...
  const { score, scoreLabel, scoreColor, factors } = useMemo(() => {
    ...
    // Base 300
    // + Utilization (30% weight): < 10% = 250, < 30% = 200, < 50% = 150, else 100
    const util = summary.utilizationPct;
    const utilScore = util < 10 ? 250 : util < 30 ? 200 : util < 50 ? 150 : 100;
    
    // + Payment History (35% weight): Mocked 300
    // + Age (15% weight): Mocked 120
    // + Variety (10% weight): based on accounts count
    const varietyScore = summary.activeCount > 5 ? 100 : summary.activeCount > 2 ? 80 : 50;
    // + New Credit (10% weight): Mocked 80
    
    const finalScore = 300 + utilScore + 180 + 100 + varietyScore + 60; // Simplified
    const limitedScore = Math.min(850, Math.max(300, finalScore));

    const labelObj = scoreRanges.find(r => {
    ...
    const currentFactors = [
      ...
      { label: 'Utilisasi Kredit',   status: `${util.toFixed(1)}% (${util < 30 ? 'Baik' : 'Tinggi'})`,  color: util < 30 ? 'success' : util < 60 ? 'warning' : 'danger', icon: util < 30 ? 'check' : 'alert-circle', weight: 30, progress: Math.max(0, 100 - util) },
      { label: 'Usia Kredit',        status: '3.5 tahun',   color: 'warning', icon: 'minus', weight: 15, progress: 55 },
      { label: 'Variasi Kredit',     status: summary.activeCount > 3 ? 'Beragam' : 'Terbatas',     color: summary.activeCount > 3 ? 'primary' : 'warning', icon: 'minus', weight: 10, progress: summary.activeCount * 20 },
        ...
        <div className="col-12 col-md-3">
          <div className="card border-0 shadow-sm h-100">
            ...
            <div className="card-body d-flex flex-column align-items-center text-center py-4">
              <div className="position-relative d-inline-flex align-items-center justify-content-center mb-3" style={{ width: 120, height: 120 }}>
                <svg width="120" height="120" viewBox="0 0 120 120">
                  <circle cx="60" cy="60" r={R} fill="none" stroke="var(--tblr-border-color)" strokeWidth="10" />
                ...
                <div className="position-absolute text-center">
                  <div className="h2 fw-bold m-0 lh-1" style={{ color: scoreColor }}>{score}</div>
                  <div className="text-muted" style={{ fontSize: '11px' }}>/ 850</div>
                </div>
              </div>

              <span className="badge border-0 px-3 py-2 rounded-pill fw-bold mb-3" style={{ backgroundColor: `${scoreColor}20`, color: scoreColor }}>
                {scoreLabel}
              </span>
              ...
              <ul className="list-group list-group-flush w-100">
                {scoreRanges.map((r, i) => {
                  ...
                  return (
                    <li key={i} className={`list-group-item d-flex justify-content-between align-items-center px-0 py-1 ${active ? 'fw-semibold' : ''}`}>
                      <div className="d-flex align-items-center gap-2">
                        <span className="rounded-circle flex-shrink-0" style={{ width: 7, height: 7, backgroundColor: r.color, display: 'inline-block' }} />
                        <span className={`small ${active ? 'text-body' : 'text-muted'}`}>{r.label}</span>
                      </div>
              ...
              {factors.map((f, i) => (
                <div key={i}>
                  <div className="d-flex align-items-center justify-content-between mb-1">
                    ...
                    <span className={`badge bg-${f.color}-lt text-${f.color} border-0 rounded-1`} style={{ fontSize: '10px' }}>
                      {f.status}
                    </span>
                  </div>
            ...
            <div className="card-body d-flex flex-column gap-3">
              <Chart
                ...
                chartData={{
                  type: 'bar',
                  stacked: false,
                  series: [{ name: 'Skor', color: 'var(--tblr-primary)', data: [score - 40, score - 30, score - 25, score - 15, score - 5, score] }],
                  categories: ['Des', 'Jan', 'Feb', 'Mar', 'Apr', 'Mei'],
                  datalabels: false,
                  legend: false,
                  grid: { strokeDashArray: 4, borderColor: 'var(--tblr-border-color)', padding: { top: 0, right: 0, bottom: 0, left: 0 } },
                  xaxis: { axisBorder: { show: false }, labels: { style: { colors: 'var(--tblr-secondary)', fontSize: '11px' } } },
                  yaxis: { show: false, min: 300, max: 850 },
                  extend: { plotOptions: { bar: { borderRadius: 4, columnWidth: '45%' } }, tooltip: { theme: 'dark' } },
                }}
              />
              ...
              <div className="alert alert-success d-flex align-items-start gap-2 mb-0 py-2">
                ...
                <div>
                  ...
                  <div className="small">Skor Anda <strong>{score}</strong> dengan utilisasi <strong>{summary.utilizationPct.toFixed(1)}%</strong>.</div>
                </div>
              </div>
              ...
              <div className="alert alert-warning d-flex align-items-start gap-2 mb-0 py-2">
                ...
                <div>
                  ...
                  <div className="small">{summary.utilizationPct > 30 ? 'Turunkan utilisasi di bawah 30% untuk skor 750+.' : 'Jaga konsistensi pembayaran untuk capai 800+.'}</div>
                </div>
              </div>
            </div>
          </div>
