...
export function TransactionsListCard({ 
  ...
  const lastPage = txData?.last_page || 1
  ...
  // Kapanpun tipe atau sorting berubah, reset page ke 1
  const handleTypeChange = (val: 'all' | TransactionType) => {
    setActiveType(val)
    setPage(1)
  }
  ...
  return (
    <div>
      {/* Filter type chips */}
      {!hideFilters && (
        <div className="d-flex gap-2 overflow-auto pb-2 mb-3 hide-scrollbar" style={{ WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none' }}>
          {TYPE_FILTERS.map(f => {
            ...
            return (
              <button
                ...
                style={{
                  backgroundColor: isActive ? 'var(--tblr-primary)' : 'var(--tblr-bg-surface)',
                  color: isActive ? '#ffffff' : 'var(--tblr-body-color)',
                  border: isActive ? 'none' : '1px solid var(--tblr-border-color)',
      ...
      {!hideHeader && (
        <div className="d-flex justify-content-between align-items-center mb-3">
          ...
          <div className="position-relative">
            <button
              className="d-flex align-items-center bg-surface text-body px-2 py-1 rounded-pill border gap-1"
              style={{ fontSize: '12px' }}
            ...
            {showSortMenu && (
              <div
                className="position-absolute end-0 bg-surface border rounded-3 shadow-sm py-1"
              ...
              <div key={tx.id} className="card border-0 rounded-4 shadow-sm">
                <div className="card-body p-3 d-flex justify-content-between align-items-center gap-2">
                  <div className="flex-grow-1 overflow-hidden">
                    <div className="fw-semibold text-body mb-1 text-truncate" style={{ fontSize: '14px' }}>
                      {tx.merchant || tx.category?.name || (isIncome ? 'Pemasukan' : isExpense ? 'Pengeluaran' : 'Transfer')}
                    </div>
                    <div className="d-flex align-items-center gap-1 flex-wrap text-secondary" style={{ fontSize: '11px' }}>
                      {tx.account?.name && (
                        <span
