    ...
    items: [
      {
        ...
        type: 'Investment',
        color: '#41B549',
        logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/1/1a/Bibit_logo.svg/512px-Bibit_logo.svg.png',
      },
    ...
    <Modal show={show} onClose={onClose} size="md">
      <ModalHeader onClose={onClose}>
        <div className="d-flex align-items-center gap-2">
          ...
          <Icon
            icon={step === 'selection' ? 'list-details' : 'wallet'}
              ...
              {ACCOUNT_CATEGORIES.find((c) => c.id === activeTab)?.items.map((item) => (
                <label key={item.value} className="form-selectgroup-item flex-fill mb-0">
                  <input
                    ...
                    checked={
                      selectedPredefined?.value === item.value ||
                      (isCustom && item.value === 'Custom')
                    }
                  />
                  <div className="form-selectgroup-label d-flex align-items-center p-3 text-start cursor-pointer">
                    ...
                    <div className="d-flex align-items-center gap-3">
                      {item.logo ? (
                        ...
                        <div className="avatar avatar-sm" style={{ backgroundColor: item.color }}>
                          <Icon icon="wallet" className="text-white" size={16} />
                        </div>
                      )}
                      <span className="fw-bold text-body" style={{ fontSize: '1.05rem' }}>
                        {item.label}
                      </span>
                    </div>
                  </div>
          ...
          <div className="modal-body py-4">
            <div className="row g-3">
                  ...
                  >
                    {selectedPredefined?.logo ? (
                      ...
                      <div
                        ...
                        style={{ backgroundColor: selectedPredefined?.color || '#000' }}
                      ></div>
                    )}
              ...
              <div className={isCustom ? 'col-md-6' : 'col-12'}>
                ...
                {errors.balance && (
                  <div className="invalid-feedback d-block">{errors.balance.message}</div>
                )}
              </div>
              ...
              {isCustom && (
                <div className="col-md-6">
                  <label className="form-label fw-medium">
                    Link Logo <span className="text-muted fw-normal">(Opsional)</span>
                  </label>
                  <input
                    type="text"
                    {...register('logo')}
