          ...
          <div>
            <div className="row g-2">
              {ccAccounts.map((c, i) => {
                ...
                const dueDate = c.credit?.due_date ? new Date(c.credit.due_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }) : 'N/A';
                    ...
                    <div className="card border shadow-none bg-transparent h-100">
                      <div className="card-body p-2 d-flex flex-column gap-1">
                        <div className="d-flex justify-content-between align-items-start">
                          <div className="fw-bold small text-truncate" style={{ maxWidth: '80%' }}>{c.name}</div>
                          <span className="badge bg-azure-lt border-0" style={{ fontSize: '8px' }}>CC</span>
                        </div>
                        <div className="text-muted" style={{ fontSize: '11px' }}>{fmt(c.credit!.total_amount)} / {fmt(c.credit!.limit)}</div>
                        ...
                        <div className="mt-2 d-flex flex-column gap-1">
                          ...
                          <div className="d-flex justify-content-between" style={{ fontSize: '10px' }}>
                            ...
                            <span className="fw-medium">{fmt(Math.round(c.credit!.total_amount * 0.1))}</span>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        infoBox: (
          <div className="alert alert-azure mb-0 mt-2 p-2 border-0">
            <div className="small d-flex justify-content-between align-items-center">
              ...
              <strong className={ccPct > 40 ? 'text-warning' : 'text-azure'}>{ccPct.toFixed(1)}%</strong>
            </div>
          </div>
        ),
                  ...
                  <div key={i} className={colClass}>
                    <div className="card border shadow-none bg-transparent h-100">
                      <div className="card-body p-2 d-flex flex-column gap-1">
                        <div className="d-flex justify-content-between align-items-start">
                          <div className="fw-bold small text-truncate" style={{ maxWidth: '80%' }}>{c.name}</div>
                          <span className="badge bg-green-lt border-0" style={{ fontSize: '8px' }}>PL</span>
                        </div>
                        <div className="text-muted" style={{ fontSize: '11px' }}>{fmt(c.credit!.total_amount)} / {fmt(c.credit!.limit)}</div>
                        <div className="progress progress-sm" style={{ height: '4px' }}>
                          <div className={`progress-bar bg-${pct > 60 ? 'danger' : pct > 30 ? 'warning' : 'success'}`} style={{ width: `${pct}%` }} />
          ...
          <div className="mt-2">
            <div className="alert alert-green mb-0 p-2 border-0 opacity-75">
              <div className="small d-flex justify-content-between align-items-center">
                ...
                <strong className={plPct > 40 ? 'text-warning' : 'text-green'}>{plPct.toFixed(1)}%</strong>
              </div>
            </div>
          </div>
        ),
              ...
              <div className={`card border-0 shadow-sm h-100 ${isEmpty ? 'opacity-75' : ''}`}>
                <div className="card-header py-2 px-3">
                  <div className="d-flex align-items-center gap-2">
                    <span className={`avatar avatar-sm bg-${isEmpty ? 'secondary' : ct.color} text-white`} style={{ borderRadius: '10px' }}>
                      <Icon icon={ct.icon} size={16} />
                    </span>
                    <div>
                      <div className="card-title fw-bold mb-0" style={{ fontSize: '13px' }}>{ct.title}</div>
                      <div className="text-muted" style={{ fontSize: '11px' }}>{isEmpty ? 'Belum ada jalur aktif' : ct.subtitle}</div>
                    </div>
                  </div>
                </div>
                <div className="card-body p-3">
                  {isEmpty ? (
                    ...
                    <>
                      {ct.customBody ? ct.customBody : (
                        <>
                          <ul className="list-group list-group-flush mb-2">
                            {(ct.rows ?? []).map((row, i) => (
                              <li key={i} className="list-group-item d-flex justify-content-between align-items-center px-0 py-1 border-0">
                          ...
                          {ct.progress !== undefined && (
                            <div className="mt-2">
                              ...
                              <div className="text-muted" style={{ fontSize: '10px' }}>{ct.progressLabel}</div>
                            </div>
                          )}
                        </>
                      )}
