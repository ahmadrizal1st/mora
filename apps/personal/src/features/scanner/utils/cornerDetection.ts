  ...
  for (let y = 2; y < h - 2; y++) {
    for (let x = 2; x < w - 2; x++) {
      ...
      for (let dy = -2; dy <= 2; dy++)
        for (let dx = -2; dx <= 2; dx++, ki++) s += gray[(y + dy) * w + (x + dx)] * k[ki]
      out[y * w + x] = (s / kSum) | 0
    }
  }
  return out
  ...
  for (let y = 1; y < h - 1; y++)
    for (let x = 1; x < w - 1; x++) {
      let s = 0,
        ki = 0
      for (let dy = -1; dy <= 1; dy++)
        for (let dx = -1; dx <= 1; dx++, ki++) s += gray[(y + dy) * w + (x + dx)] * k[ki]
...
export function largestConnectedComponent(mask: Uint8Array, w: number, h: number): Uint8Array {
  ...
  for (let x = 0; x < w; x++) {
    const tl = labels[x]
    if (tl >= 0) borderLabels.add(tl)
    const bl = labels[(h - 1) * w + x]
    if (bl >= 0) borderLabels.add(bl)
  }
  for (let y = 0; y < h; y++) {
    const ll = labels[y * w]
    if (ll >= 0) borderLabels.add(ll)
    const rl = labels[y * w + w - 1]
    if (rl >= 0) borderLabels.add(rl)
  }
  ...
  for (let t = 0; t < tSteps; t++)
    for (let r = 0; r < 2 * rMax + 1; r++)
      if (acc[t * (2 * rMax + 1) + r] > threshold)
        lines.push({
          theta: (t * Math.PI) / tSteps,
          rho: r - rMax,
          votes: acc[t * (2 * rMax + 1) + r],
        })
